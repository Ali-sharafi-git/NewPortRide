            ...
            formik.values.service_name === t("airport_transfers")
              ? `<div>
                  ...
                  values.phone_number
                        ...
                        <p><strong>${t("service")}:</strong> ${
                  values.service_name
                }</p>
                        <p><strong>${t("meet_and_greet")}:</strong> ${
                  values.greet !== "" ? t("yes") : t("no")
                }</p>
    ...
    if (Object.keys(formik.errors).length > 0) {
      toast.error(
        `${formik.errors.full_name || ""} ${
          formik.errors.email || ""
        }`.trim() || t("please_enter_your_name_and_email")
      );
    }
    formik.handleSubmit();
  };
    ...
    <form className="grid grid-cols-1 md:grid-cols-2 gap-7">
      ...
      {formik.values.service_name === t("airport_transfers") && (
        <FormItem
          type="checkbox"
          name="greet"
          label={t("meet_and_greet")}
          formik={formik}
          showLabel={false}
          parentClassName="col-span-full"
          inputProps={{ value: t("yes") }}
        />
      )}
      <FormItem
        type="input"
